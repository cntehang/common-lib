...
public class BjTimeToSecond extends BjDateTime implements Serializable {
  ...
  private static final DateTimeFormatter PATTERN = DateTimeFormat.forPattern(DATE_FORMAT_TO_SECOND).withZone(DateTimeZone.forID(ZONE_SHANGHAI));
  ...
  /**
   * 从UTC时间格式字符串转为BjTimeToSecond
   */
  public static BjTimeToSecond parseFromUtc(String utcDateTime) {
    checkUtcTimeFormat(utcDateTime);

    var dateTime = DateTime.parse(utcDateTime);
    return new BjTimeToSecond(dateTime.toDateTime(DateTimeZone.forID(ZONE_SHANGHAI)).toString(PATTERN));
  }
   ...
   * 将BjTimeToSecond序列化为Json.
   */
  public static class UtcFormatSerializer extends JsonSerializer<BjTimeToSecond> {
    @Override
    public void serialize(BjTimeToSecond value, JsonGenerator gen, SerializerProvider serializers) throws IOException {
      if (value == null) {
        gen.writeNull();
   ...
   * 将json反序列化为BjTimeToSecond.
   */
  public static class UtcFormatDeserializer extends JsonDeserializer<BjTimeToSecond> {
    @Override
    public BjTimeToSecond deserialize(JsonParser p, DeserializationContext ctx) throws IOException, JsonProcessingException {
      String text = p.getText();
      return isBlank(text)
          ? null
          : BjTimeToSecond.parseFromUtc(text);
    }
  }
}
