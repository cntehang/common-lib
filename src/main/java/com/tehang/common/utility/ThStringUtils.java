...
public final class ThStringUtils {
  ...
  public static boolean equalsAny(final CharSequence cs, final CharSequence... searchCharSequences) {
    ...
    if (StringUtils.isNotEmpty(cs) && ArrayUtils.isNotEmpty(searchCharSequences)) {
      result = Arrays.stream(searchCharSequences).anyMatch(searchCharSequence -> StringUtils.equals(cs, searchCharSequence));
    }
    return result;
  }
  ...
  public static boolean equalsAnyIgnoreCase(final CharSequence cs, final CharSequence... searchCharSequences) {
    boolean result = false;
    if (StringUtils.isNotEmpty(cs) && ArrayUtils.isNotEmpty(searchCharSequences)) {
      result = Arrays.stream(searchCharSequences).anyMatch(searchCharSequence -> StringUtils.equalsIgnoreCase(cs, searchCharSequence));
    }
    return result;
  }
  ...
  public static String joinWith(Collection<String> items, String separatorChar) {
    return CollectionUtils.emptyIfNull(items).stream().filter(StringUtils::isNotBlank).collect(Collectors.joining(separatorChar));
  }
