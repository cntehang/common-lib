...
public final class JwtUtils {
   ...
   * 生成.
   *
   * <p>生成一个新的 JWT Token 该 Token 被使用的时间需要在其 issueAt 时间之后，若服务器之间存在时钟差，
   * 有概率出现 The token can't be used before 问题 故对 issueAt 取当前时间的三分钟前，
   * 避免这种由于服务器间时钟的微小差距带来的Token 无法正常使用问题
   */
  public static String createToken(InnerJwtPayload payload, String secret, String issuer, String aud) throws UnsupportedEncodingException {
    try {
      ...
      return JWT.create().withIssuer(issuer).withAudience(aud).withClaim(CUSTOMER_PAYLOAD, payload.toString()).sign(Algorithm.HMAC256(secret));
    }
