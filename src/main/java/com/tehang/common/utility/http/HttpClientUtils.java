...
public final class HttpClientUtils {
  ...
  /**
   * Retry when remote returns SC_BAD_GATEWAY. It happens when the proxy server cannot connect to supplier.
   * We set the proxy server's TCP connection timeout to a small number and the HTTP proxy returns SC_BAD_GATEWAY.
   * <p>
   * SC_BAD_GATEWAY may misfire for other errors, hopefully it is not often because retry is not free.
   */
  ...
  /**
   * 获取 HttpPost 请求实体
   * 默认采用"application/json; charset=utf-8"作为content type
   *
   * @param url      请求地址
   * @param postData 请求数据
   * @return
